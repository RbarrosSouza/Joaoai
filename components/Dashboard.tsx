   ...
   const aiInsight = useMemo(() => {
      ...
      transactions.forEach(t => {
         ...
         if (t.type === TransactionType.EXPENSE && !t.isPending && d.getMonth() === currentMonth && d.getFullYear() === currentYear) {
            const { category } = resolveCategory(t.categoryId, categories);
            const catId = category?.id || t.categoryId;
            currentMonthExpenses[catId] = (currentMonthExpenses[catId] || 0) + t.amount;
         }
      });
   ...
   const categorySpending = useMemo(() => {
      const stats = categories.map(cat => {
         const total = transactions
            .filter(t => {
               const { category } = resolveCategory(t.categoryId, categories);
               return category?.id === cat.id && t.type === TransactionType.EXPENSE && !t.isPending;
            })
         ...
         <header className="flex items-center justify-between gap-4 animate-slide-up pt-safe">
            ...
            <div className="flex items-center gap-3 self-start md:self-auto shrink-0">
               ...
               <button
                  ...
                  className={`relative p-3 rounded-full border shadow-sm transition-all group active:scale-95 ${isPrivacyMode
