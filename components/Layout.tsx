...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const NavItem = ({ path, icon: Icon, label }: { path: string, icon: any, label: string }) => (
    <button
      ...
      className={`flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all duration-300 w-full text-left group ${
        isActive(path) 
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-brand-background text-slate-800">
      ...
      <main className="flex-1 h-full overflow-y-auto overflow-x-hidden relative scroll-smooth bg-brand-background">
        <div className="w-full max-w-[1600px] mx-auto h-full flex flex-col p-6 md:p-12 pb-24 md:pb-12">
           {children}
        </div>
      </main>
